...
function sendApiRequest(x) {
  //Where Ryan+Gosling is located replace it with x
    fetch("https://api.giphy.com/v1/gifs/search?api_key=dJertxev3XKxnByVqSNpNQi4f1D3VIGD&q="+x+"&limit=25&offset=0&rating=G&lang=en")
    .then(function(data) {
      return data.json();
    })
    .then(function(json) {
      console.log(json);
      //Beneath this line: pass the JSON on to the next function. HINT: Call  and pass in json as your argument.
...
function addImageToScreen(myURL) {
  ...
  let hope = document.querySelector("#wrapper")
